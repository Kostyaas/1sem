...
int main(int argc, char **argv)
{
    ...
    auto id1 = scheduler.add([](float a, float c)
                             { return -4 * a * c; }, a, c);
    ...
    auto id3 = scheduler.add([](float b, float d)
                             { return -b + std::sqrt(d); }, b, scheduler.getFutureResult<float>(id2));

    auto id4 = scheduler.add([](float b, float d)
                             { return -b - std::sqrt(d); }, b, scheduler.getFutureResult<float>(id2));
    ...
    auto id7 = scheduler.add(&AddNumber::add, add, 5);
    scheduler.executeAll();
    std::cout << "x1 = " << id5->value <<  std::endl;
